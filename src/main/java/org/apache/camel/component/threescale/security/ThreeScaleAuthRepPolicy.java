...
public class ThreeScaleAuthRepPolicy implements AuthorizationPolicy {

	private static final Logger LOG = LoggerFactory.getLogger(ThreeScaleAuthRepPolicy.class);

	private ServiceApi serviceApi;
	private CacheManager cacheManager = null;

	public ThreeScaleAuthRepPolicy(String host, int port, CachingProvider cachingProvider) {
	...
	public Processor wrap(RouteContext routeContext, final Processor processor) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Securing route {} using 3scale authrep {}", routeContext.getRoute().getId(), this);
		}

		return new ThreeScaleAuthRepProcessor(processor, this);
