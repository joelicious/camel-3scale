...
public class ThreeScaleAuthProcessor extends ThreeScaleBaseProcessor {

	private static final Logger LOG = LoggerFactory.getLogger(ThreeScaleAuthProcessor.class);
	private final ThreeScaleAuthPolicy policy;

	public ThreeScaleAuthProcessor(Processor processor, ThreeScaleAuthPolicy policy) {
		super(processor);
		this.policy = policy;
	}
	...
	private boolean invoke3scale(ThreeScaleConfiguration threeScaleConfiguration) {
		...
		try {
			...
			response = this.policy.getServiceApi().authorize(threeScaleConfiguration.getParameterMap());
